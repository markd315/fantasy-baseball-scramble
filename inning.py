...
def simBlendedInning(battingTeam, pitchingTeam, orderSlot, currPitcher, inning, pitcherScore, batterScore, pitcherHome):
    if inning > 9:
        baseState = [0, 1, 0]
    else:
        baseState = [0, 0, 0]
    ...
    while outs < 3:
        ...
        if coin == 0:
            ...
            while len(team['pitching-results'][currPitcher]) < 2:
                old = currPitcher
                ...
                if inning >= 7:
                    if inning >= 9 and team['closer'] not in team[
                        'burned-pitchers']:
                        if (pitcherHome and score_d <= team[
                            "closer-max-lead-home"] and score_d >= team[
                                "closer-min-lead-home"]) or (
                                not pitcherHome and score_d >= -1 * team[
                        ...
                        if (pitcherHome and score_d <= team[
                            ...
                            logs += currPitcher + " is sweating and the manager makes a call to the pen, luckily " + \
                                    team[
                                        'fireman'] + " was already warming up.\n"
    ...
    return {"out": logs, "runs": runs, "orderSlot": orderSlot, "currPitcher": currPitcher}
